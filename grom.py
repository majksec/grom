...
print("[+] Built with love by ~ Elon Dusk & majksec")
...
class threadedFileCheck:
    ...
    def threadCheckLine(self, clean_line): #returns responsive sites as list with response codes
        ...
        try:
            response = requests.get(clean_line)

            if response.status_code == 200:
                
                print("[+] {} is up {}".format(clean_line, response))

                self.pLock.acquire()

                self.responseQueue.put(clean_line + " " + str(response))

                self.pLock.release()
            else:
                print("[-] {} is up with code {}".format(clean_line, response.status_code))

        except:
            print("[-] seems like {} is down or failed SSL auth.".format(clean_line))
